...
export async function GET(
    ...
    try {
        ...
        console.log("[ADMIN_QUIZ_GET] Fetching quiz:", resolvedParams.quizId, "for user:", userId);

        if (!userId) {
            return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
        }

        // Get the quiz; admin can access all quizzes
        ...
        if (!quiz) {
            console.log("[ADMIN_QUIZ_GET] Quiz not found for ID:", resolvedParams.quizId);
            return NextResponse.json({ error: "Quiz not found" }, { status: 404 });
        }

        console.log("[ADMIN_QUIZ_GET] Quiz found:", quiz.id, "with", quiz.questions.length, "questions");
        ...
        const quizWithParsedOptions = {
            ...
            questions: quiz.questions.map(question => {
                try {
                    ...
                    console.log("[ADMIN_QUIZ_GET] Error parsing options for question:", question.id, parseError);
                    return {
                        ...question,
        ...
        console.log("[ADMIN_QUIZ_GET] Error details:", error);
        console.log("[ADMIN_QUIZ_GET] Error stack:", (error as Error).stack);
